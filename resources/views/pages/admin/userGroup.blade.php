...
@section('body')
    ...
    <div class="card border-primary">
        <div class="card-header bg-primary text-white">
            <div class="row">
                <form class="col-12 col-sm-1 col-md-1 text-center" method="POST" action="{{ route('admin.users.list') }}">
                    @csrf
        ...
        <div class="card-body">
            <ul class="list-group">
                @forelse ($groupsUser as $item)
                    <li class="list-group-item text-primary border-primary">
                        <div class="d-flex w-100 justify-content-between">
                            <h5><i class="{{ $item->icon }}"></i></h5>
                            <h6 class="mb-1">
                                {{ $item->name }}
                            </h6>
                            <form method="POST" action="{{ route('admin.users.removeGroup') }}">
                                @csrf
                                <input type="hidden" name="idUser" value="{{ $idUser->id }}">
                                <input type="hidden" name="idGroup" value="{{ $item->id_users_group }}">
    ...
    <div class="modal fade" id="addGroup" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST" action="{{ route('admin.users.addGroup') }}" class="modal-content was-validated" autocomplete="off">
                        ...
                        <div class="form-group col-sm-12 col-12 col-md-12">
                            ...
                            <select class="form-control" id="idGroup" name="idGroup" required>
                                @foreach($groups as $group)
                                    <option value="{{ $group->id_users_group }}">{{ $group->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
