    ...
    class Contracts extends Controller
    {
        public function list(Request $request) {
            try {
                ...
                foreach ($contracts as $key => $value) {
                    ...
                    $contracts[$key]->allDesc->type             =   TransactionType::find($value->id_transaction_type)->description;
        ...
        public function createPage(Request $request) {
            try {
                ...
                $types  =   TransactionType::orderBy('description','asc')->get();
        ...
        public function add(Request $request) {
            try {
                ...
                $contract->id_transaction_type  =   $request->type;
                ...
                if(isset($request->address) && count($request->address) > 0) {
                    foreach ($request->address as $key => $value) {
                        ...
                        $contractAddress->zip_code   =   $request->postal_code[$key];
                        $contractAddress->save();
                    } // foreach ($request->address as $key => $value) { ... }
                } // if(isset($request->address) && count($request->address) > 0) { .. }

                if(isset($request->phone) && count($request->phone) > 0) {
                    foreach ($request->phone as $key => $value) {
                        $contractPhone              =   new ContractPhone;
                        $contractPhone->id_contract =   $contract->id_contract;
                        $contractPhone->reference   =   $request->reference[$key];
        ...
        public function id(Request $request) {
            try {
                ...
                $contract->allDesc->type            =   TransactionType::find($contract->id_transaction_type)->description;
